...
def pde(X, V):
    ...
    dfx_x = dde.grad.jacobian(V, X, i=3, j=0)
    dfy_y = dde.grad.jacobian(V, X, i=4, j=1)
...
def generate_training_points(x, y, u, v):
    x_t = []
    y_t = []
    u_t = []
    v_t = []
    ...
    for i in range(x_t.shape[0]):
        if geom.inside([x_t[i, 0], y_t[i, 0]]) \
            ...
            X.append([x_t[i, 0], y_t[i, 0], u_t[i, 0], v_t[i, 0]])
    
    X = np.array(X)
 
    return np.hsplit(X, 4)

[x_training, y_training, u_training, v_training] = \
    generate_training_points(new_x, new_y, new_u, new_v)
...
data = dde.data.PDE(
    geom, pde, 
    [bc_wall_u, bc_wall_v, bc_wall_fx, bc_wall_fy, u_training_points, v_training_points], 
...
dde.saveplot(losshistory, train_state, issave=True, isplot=False, loss_fname="vel_only_loss", output_dir="Losses")
model.save("models/vel_only")

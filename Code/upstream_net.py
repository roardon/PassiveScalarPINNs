...
grid_x, grid_y = np.mgrid[-14.75:39.85:221j, -9.9:9.9:81j]
...
for k in range(4):
  layer_size = [2] + [100+((k%2)*20)] * (8+(k//2)) + [7]
  activation = "tanh"
  ...
  dde.saveplot(losshistory, train_state, issave=True, isplot=False, loss_fname="upstream_net_loss_"+str(100+((k%2)*20))+"_"+str(8+(k//2)), output_dir="Losses")
  model.save("models/upstream_net_" + str(100+((k%2)*20)) + "_" + str(8+(k//2)))
